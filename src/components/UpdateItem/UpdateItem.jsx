...
const UpdateItem = () => {
  ...
  const isOwner = user_email === user.email;
  const handleUpdateCraft = (event) => {
    ...
    if (isOwner) {
        ...
        .then((data) => {
          ...
          if (data.modifiedCount > 0) {
            Swal.fire({
              ...
              icon: "success",
              confirmButtonText: "Go Back My List",
            }).then((result) => {
      ...
      Swal.fire({
        title: "Error!",
        text: "You are not authorized to update this product.",
        icon: "error",
        confirmButtonText: "Retry again your my list",
      }).then((result) => {
        if (result.isConfirmed) {
          // Redirect to the my list page when "Go Back My List" is clicked
  ...
  return (
    <>
      <Helmet>
        <title>Update Item | Art & Craft</title>
      </Helmet>
      <div className="p-4">
